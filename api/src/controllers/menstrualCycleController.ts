...
router.get("/optionRemovePeriod/:date", async (req, res, next) => {
  ...
  if (jwtTokenResult == null) return next(new AppError("Invalid token!", 400));
  return res.json(
    await menstrualCycleService.canRemovePeriod(jwtTokenResult.id, new Date(req.params.date))
  );
});
...
router.get("/optionEndPeriod/:date", async (req, res, next) => {
  ...
  return res.json(
    await menstrualCycleService.canEndPeriod(jwtTokenResult.id, new Date(req.params.date))
  );
});
...
router.get("/optionAddPeriod/:date", async (req, res, next) => {
  ...
  return res.json(
    await menstrualCycleService.canAddPeriod(jwtTokenResult.id, new Date(req.params.date))
  );
});
